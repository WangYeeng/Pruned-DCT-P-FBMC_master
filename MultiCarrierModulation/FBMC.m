...
%             ReceivedSignal_CorrespondingSamplesToFBMCSymbol=reshape(ReceivedSignal(obj.Implementation.IndexNumberAfterIFFT(:)),[obj.Nr.SamplesPrototypeFilter obj.Nr.MCSymbols]);
        ...
        function CodingMatrix = GetPrecodingMatrixForQAMinOQAM(obj,TimeSpreading,StartIndex,Transform)
            % Chose between time or frequency
            switch TimeSpreading
                case 'Frequency'
                    error('Frequency domain pre-coding is not supported.');
                case 'Time'
                ...
                switch Transform  
                    case 'DCT' 
                Matrix = dct(eye(obj.Nr.Subcarriers));
                    case 'DST'
                Matrix = dst(eye(obj.Nr.Subcarriers))/sqrt(obj.Nr.Subcarriers);
                end
                CodingMatrix_Basis(:,:,1) = Matrix(:,1:2:end);
                CodingMatrix_Basis(:,:,2) = Matrix(:,2:2:end);
